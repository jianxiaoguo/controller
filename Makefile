...
podman-build: check-podman
	podman build ${podman_BUILD_FLAGS} -t ${IMAGE} rootfs
	podman tag ${IMAGE} ${MUTABLE_IMAGE}

podman-buildx: check-podman
	podman buildx build --platform ${PLATFORM} -t ${IMAGE} rootfs --push

podman-build-test: check-podman
	podman build ${podman_BUILD_FLAGS} -t ${IMAGE}.test -f rootfs/Dockerfile.test rootfs
