...
# 全局 session 变量
session = None
drycc_version = "2.3.0"  # 可以根据实际情况修改


def user_agent(component: str, version: str) -> str:
    """生成 User-Agent 字符串"""
    return f"{component}/{version} (Python/{'.'.join(map(str, sys.version_info[:3]))})"


def get_k8s_session():
    """获取或创建 Kubernetes API Session"""
    global session
    if session is None:
...
class K8sAPIMonitor:
    ...
    def init_session(self) -> bool:
        """初始化 Session"""
        try:
            self.session = get_k8s_session()
            return True
        except FileNotFoundError as e:
            self.write_log(f"ERROR: Token 文件不存在 - {str(e)}")
            return False
        except Exception as e:
            self.write_log(f"ERROR: 初始化 Session 失败 - {str(e)}")
            return False
    ...
    def print_header(self):
        """打印日志头部"""
        user_agent_str = self.session.headers.get('User-Agent', 'N/A') if self.session else 'N/A'
        verify_cert = self.session.verify if self.session else 'N/A'
        
        header = (
            f"\n{'='*130}\n"
            f"开始监控 Kubernetes API\n"
            f"监控端点数: {len(self.api_urls)}\n"
            f"监控时长: {self.duration_minutes} 分钟\n"
            f"请求间隔: {self.interval_seconds} 秒/端点\n"
            f"User-Agent: {user_agent_str}\n"
            f"SSL 证书: {verify_cert}\n"
    ...
    def print_summary(self, total_request_count: int, total_success_count: int, total_fail_count: int):
        ...
        if total_request_count > 0:
            success_rate = (total_success_count / total_request_count * 100)
            fail_rate = (total_fail_count / total_request_count * 100)
            self.write_log(f"  总请求次数: {total_request_count}")
            self.write_log(f"  成功次数: {total_success_count} ({success_rate:.2f}%)")
            self.write_log(f"  失败次数: {total_fail_count} ({fail_rate:.2f}%)")
        else:
            self.write_log("  总请求次数: 0")
    ...
    def run(self, target_urls: Optional[List[str]] = None):
        ...
        if not self.init_session():
            self.write_log("ERROR: Session 初始化失败，程序退出")
            return
        ...
        try:
            ...
            self.write_log(f"\n\nERROR: 监控过程出现异常 - {str(e)}")
            self.print_summary(total_request_count, total_success_count, total_fail_count)


def main():
    """主函数"""
    # 方式1: 监控所有 API（默认配置：5分钟，每10秒）
    ...
    # 方式4: 长时间监控（30分钟，每15秒）
    # monitor = K8sAPIMonitor(duration_minutes=30, interval_seconds=15)
    # monitor.run()
